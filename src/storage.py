...
- Garantir segurança estrita contra Path Traversal, bytes nulos e corrupção de arquivos.
- Controle de concorrência com trava de arquivo no SO (fcntl.flock) e threading.RLock.
"""
...
try:
    ...
    fcntl = None  # Fallback para ambientes não-POSIX
import json
...
class StorageManager:
    """Gerenciador de persistência local em JSON com trava inter-processos e proteção contra Path Traversal."""
    ...
    def __init__(self, data_dir: Optional[Path] = None) -> None:
        ...
        self.lock_file_path = self.data_dir / ".storage.lock"

    @contextmanager
    def _file_lock(self):
        """Context manager que aplica trava de thread e trava exclusiva de arquivo (flock)."""
        with self._thread_lock:
            lock_fd = None
            if fcntl is not None:
                try:
                    lock_fd = open(self.lock_file_path, "a+")
                    try:
    ...
    # ========================================================================
    # Histórico de Conversões (Thread e Process-Safe)
    # ========================================================================

    def save_conversion_record(self, record: ConversionRecord) -> None:
        """Salva um registro no histórico de conversões de forma segura e sincronizada."""
    ...
    # ========================================================================
    # Moedas Favoritas (Thread e Process-Safe)
    # ========================================================================
    ...
    def save_favorite(self, currency_code: str) -> bool:
        ...
        """
        with self._file_lock():
            favs: List[str] = self._read_json(self.favorites_file, [])
    ...
    # ========================================================================
    # Exportação Segura (CSV / JSON / Texto)
    # ========================================================================
